...
//	private Collection<UndirectedArc> undirArcs = new ArrayList<UndirectedArc>();
...
//	private ArrayList<DirectedArc> children = new ArrayList<DirectedArc>();
	...
	private HashSet<Node> nodeValues = new HashSet<Node>();//set with node values(distances)
	...
	public String toLongString () {
		return "Node<" + userObject + ">, undir arcs:" + undirArcMap.keySet().size() 
									+ ", parents:" + parentMap.keySet().size()
									+ ", children:" + childrenMap.keySet().size();
	}
...
//		parents.add(directedArc);
	...
	{
		return !(Double.isInfinite(shortestPath) || 
				Double.isInfinite(backwardShortestPath));
	}
	 ...
	 */
	 public void updateChildrenNodes()
	   ...
	   //       if (isPermanent(node)){ continue;}
	            ...
	            {
	              node.setShortestPath(shortP);// assign new shortest distance and set in setSdash
                }
	          } 
	        }
	 ...
	 public void updateBackwardChildrenNodes()
	   ...
	   //       if (isPermanent(node)){ continue;}
	            ...
	            {
	              node.setBackwardShortestPath(shortP);// assign new shortest distance and set in setSdash
             }
	          } 
	        }
